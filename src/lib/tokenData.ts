...
/**
 * Fetch token data from Solscan API (requires API key)
 */
...
export async function fetchTokenData(
  ...
  // Fallback to Solscan if API key provided
  if (options?.solscanApiKey) {
    const solscanData = await fetchSolscanData(tokenAddress, options.solscanApiKey);
    if (solscanData) {
      const onChainData = await fetchOnChainMetadata(connection, tokenAddress);
      // Solscan doesn't provide market cap directly, would need separate price API call
      return {
        name: solscanData.name || 'Unknown Token',
        symbol: solscanData.symbol || 'UNKNOWN',
        decimals: solscanData.decimals || 9,
        market_cap: 0, // Would need to calculate from price * supply
        pfp_url: solscanData.logoURI,
      };
    }
  }
